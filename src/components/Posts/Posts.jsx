...
import { fetchPostsAction, toggleAddLikesToPost, toggleAddDisLikesToPost, savePostAction, savedPostsAction, deleteSavedPostsAction } from "../../redux/slices/posts/postSlice";
...
export default function Posts() {
  ...
  const { postLists, loading, appErr, serverErr, likes, disLikes, searchLoading, savedPost, savedList, saved, deleted } = useSelector(state => state?.posts);
                          ...
                          <div class="w-full lg:w-3/4 px-3 mt-10 lg:mt-0 ">
                            ...
                            <div className="float-right cursor-pointer  ">
                              {savedList[0]?.post?.find((element) => element?._id.toString() === post?._id.toString()) ?
                                <FaBookmark onClick={() => {
                                  tostAlert(`"${post?.title}" unsaved successfully`)
                                  dispatch(deleteSavedPostsAction(post?._id))
                                }
                                } className="h-[40px] w-[40px] px-2 py-2 hover:bg-gray-200  border-none rounded-xl" /> :
                                <FaRegBookmark onClick={() => {
                                  tostAlert(`"${post?.title}" saved successfully`)
                                  dispatch(savePostAction(post?._id))
                                }
                                } className="h-[40px] w-[40px] px-2 py-2 hover:bg-gray-200  border-none rounded-xl" />}
                            </div>
